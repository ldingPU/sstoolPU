...
def case_3bus_droop_gfl():
    ...
    generator = [GFM_IBR(0.05,0.05,0.5,0.2,1.0,1.0,0.00,0.0,0.6,3.6,0.6,12.0,0.02,0.10,100,0.05,0.01,0.05,31.4159),
                 GFL_IBR(0.05,0.05,0.5,0.2,1.0,1.0,1.20,120,0.6,3.6,0.6,12.0,0.02,0.10,100,0.05,0.01,0.05,31.4159,100)]
...
def case_3bus_2droop():
    ...
    bus = pd.DataFrame([[1,'Bus1','PQ-Ref',13.8e3],
                        [2,'Bus2','PQ',13.8e3],
                        [3,'Bus3','PQ',13.8e3]])
    branch = [Branch(1,3,0.17,0.54),
              Branch(2,3,0.09,0.31)]
    load = [Load(3,4.30,0.80)]
    generator = [GFM_IBR(0.05,0.05,0.4464,0.0,1.0,1.0,0.00,0.00,0.4,9.6,0.3,6.0,0.02,0.10,100,0.05,0.01,0.05,31.4159),
                 GFM_IBR(0.05,0.05,0.5804,0.0,1.0,1.0,0.00,0.00,0.4,9.6,0.3,6.0,0.02,0.10,100,0.05,0.01,0.05,31.4159)]
    sysData = Data(baseVA,wbase,isFrequencyKnown,Rx,bus,branch,load,generator)    
    return sysData
